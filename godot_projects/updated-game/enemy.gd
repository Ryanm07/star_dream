...
# Tamanho de um bloco (tile) do mapa em pixels. Ajuste conforme o seu TileMap.
# Por padrão assume tiles de 32px.
@export var tile_size: int = 32
# Quantidade de blocos que definem o raio de perseguição. O inimigo
# começará a perseguir o player quando ele estiver a até este número de blocos
# de distância e deixará de perseguir se se afastar além desse limite.
const BLOCKS_TO_CHASE: int = 3
# Raio de perseguição em pixels. Calculado no _ready() como
# BLOCKS_TO_CHASE * tile_size.
...
func _ready() -> void:
	...
	# Calcula o raio de perseguição baseado na quantidade de blocos e no tamanho do tile.
	# Isso permite configurar a distância em termos de blocos em vez de pixels absolutos.
	chase_range = float(BLOCKS_TO_CHASE * tile_size)
	print("[ENEMY] Configurando chase_range para %.1f (BLOCKS_TO_CHASE=%d, tile_size=%d)" % [chase_range, BLOCKS_TO_CHASE, tile_size])
...
func setup_enemy() -> void:
	...
	# Exibe o range de perseguição calculado em termos de blocos.
	print("[ENEMY]    Chase Range (3 blocos): %.1f | Attack Range: %.1f" % [chase_range, enemy_data.attack_range])
	...
	# Configura área de detecção usando o chase_range calculado (3 blocos).
	if detection_area:
		var detection_shape: CollisionShape2D = CollisionShape2D.new()
		var circle: CircleShape2D = CircleShape2D.new()
		circle.radius = chase_range
		detection_shape.shape = circle
		detection_area.add_child(detection_shape)
		detection_area.body_entered.connect(_on_detection_body_entered)
		detection_area.body_exited.connect(_on_detection_body_exited)
		print("[ENEMY]    DetectionArea configurada (radius: %.1f)" % chase_range)
...
func process_idle() -> void:
	...
	# Se tiver alvo, verifica a distância até ele e muda para CHASE
	# apenas se estiver dentro do raio de perseguição configurado (chase_range).
	if target and is_instance_valid(target):
		var dist: float = global_position.distance_to(target.global_position)
		if dist <= chase_range:
			print("[ENEMY] Estado: IDLE → CHASE (distância: %.1f ≤ %.1f)" % [dist, chase_range])
...
func process_chase(_delta: float) -> void:
	...
	# Calcula a distância até o alvo para decidir se continua perseguindo.
	var distance: float = global_position.distance_to(target.global_position)
	# Se o alvo estiver fora do raio de perseguição, interrompe o chase e volta para Idle.
	if distance > chase_range:
		print("[ENEMY] Estado: CHASE → IDLE (distância: %.1f > %.1f)" % [distance, chase_range])
		target = null
		velocity = Vector2.ZERO
		current_state = State.IDLE
		return
	...
	# Verifica distância para ataque (usa attack_range do Enemy_Data)
...
func process_attack() -> void:
	...
	# Verifica se o alvo saiu do raio de perseguição ou apenas do alcance de ataque.
	var distance: float = global_position.distance_to(target.global_position)
	# Sai totalmente da perseguição se passar do chase_range.
	if distance > chase_range:
		print("[ENEMY] Estado: ATTACK → IDLE (distância: %.1f > %.1f)" % [distance, chase_range])
		target = null
		current_state = State.IDLE
		return
	# Se ainda estiver no raio de perseguição mas fora do alcance de ataque,
	# volta para CHASE para continuar perseguindo.
	if distance > enemy_data.attack_range:
		print("[ENEMY] Estado: ATTACK → CHASE (alvo fora do range de ataque)")
		current_state = State.CHASE
...
func take_damage(amount: float) -> void:
	...
	# Aplica defesa
	# GDScript infere Variant se não houver tipagem explícita; definimos como float
	# para evitar warnings tratados como erro.
	var damage_taken: float = max(amount - enemy_data.defense, 1.0)
...
func _on_detection_body_entered(body: Node2D) -> void:
	...
	if body.is_in_group("player") and enemy_data.behavior == "Aggressive":
		# Apenas considera o player se estiver dentro do raio de perseguição (3 blocos).
		var dist: float = global_position.distance_to(body.global_position)
		if dist <= chase_range:
			print("[ENEMY]    ✅ É o player (distância %.1f ≤ %.1f) e sou agressivo! Definindo como alvo..." % [dist, chase_range])
			target = body
			if current_state == State.IDLE:
				print("[ENEMY]    Estado: IDLE → CHASE")
				current_state = State.CHASE
		else:
			print("[ENEMY]    ⚠️ Player detectado mas fora do raio de perseguição (distância %.1f > %.1f)" % [dist, chase_range])
	else:
